//
//  ColorValueProvider.swift
//  lottie-swift
//
//  Created by Brandon Withrow on 2/4/19.
//

import CoreGraphics
import Foundation

/// A `ValueProvider` that returns a CGColor Value
public final class ColorValueProvider: AnyValueProvider {

  // MARK: Lifecycle

  /// Initializes with a block provider
  public init(block: @escaping ColorValueBlock) {
    self.block = block
    color = ColorLottie(r: 0, g: 0, b: 0, a: 1)
  }

  /// Initializes with a single color.
  public init(_ color: ColorLottie) {
    self.color = color
    block = nil
    hasUpdate = true
  }

  // MARK: Public

  /// Returns a Color for a CGColor(Frame Time)
  public typealias ColorValueBlock = (CGFloat) -> ColorLottie

  /// The color value of the provider.
  public var color: ColorLottie {
    didSet {
      hasUpdate = true
    }
  }

  // MARK: ValueProvider Protocol

  public var valueType: Any.Type {
    ColorLottie.self
  }

  public func hasUpdate(frame _: CGFloat) -> Bool {
    if block != nil {
      return true
    }
    return hasUpdate
  }

  public func value(frame: CGFloat) -> Any {
    hasUpdate = false
    let newColor: ColorLottie
    if let block = block {
      newColor = block(frame)
    } else {
      newColor = color
    }
    return newColor
  }

  // MARK: Private

  private var hasUpdate: Bool = true

  private var block: ColorValueBlock?
}
